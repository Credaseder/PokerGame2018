    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (plays < 1000)
            {
                ...
                int scoreCheck = ScoreChecker(plays);
                if (scoreCheck == 1)
                {
                    p1Wins++;
                }
            ...
            Console.WriteLine("Player 1 won: " + p1Wins + " times, Player 2 won: " + p2Wins + " times, The game ended in a draw: " + draws + " times");
            WriteToDisk.WriteRoundToDisk();
        }

        //Create Player 1 hand
        public static string[] GetPlayer1Hand(int currentHand)
        {
            string player1Hand = GameHand[currentHand].Split('|')[0];
            string[] cardsInHand = new string[5];
            for(int i = 0; i < 5; i++)
            {
                cardsInHand[i] = player1Hand.Split(' ')[i];
            }
            return cardsInHand;
        }

        //Create Player2 hand
        public static string[] GetPlayer2Hand(int currentHand)
        {
            string player2Hand = GameHand[currentHand].Split('|')[1];
            string[] cardsInHand = new string[5];
            for(int i =0; i < 5; i++)
            {
                cardsInHand[i] = player2Hand.Split(' ')[i];
            }
            return cardsInHand;
        }
        ...
        //Sets all values in list, then checks highest number
        public static int[] CheckHighestNumber(int gameNr)
        {
            List<int> player1Values = new List<int>();

            string[] player1Hand0 = GetPlayer1Hand(gameNr);
            foreach (string card in player1Hand0)
            {
                player1Values.Add(CheckValue(card));
            }

            List<int> player2Values = new List<int>();

            string[] player2Hand0 = GetPlayer2Hand(gameNr);
            foreach (string card in player2Hand0)
            {
                player2Values.Add(CheckValue(card));
            }

            int maxPlayer1 = player1Values.Max();
            int maxPlayer2 = player2Values.Max();

            int[] returnScore = new int[2];
            returnScore[0] = maxPlayer1 + 1;
            returnScore[1] = maxPlayer2 + 1;
            return returnScore;
        }

        //Check if there is a duplicate number
        public static int[] CheckPairs(int gameNR)
        {
            int[] returnScore = new int[2];
            returnScore[0] = 0;
            returnScore[1] = 0;
            ...
            for (int l = 0; l < 5; l++)
            {
                for (int p = 0; p < 13; p++)
                {
                    if (player1Values[l] == p)
                    {
                        cardsP1[p]++;
                        if (cardsP1[p] >= 3)
                        {
                            returnScore[0] = p + 1 + 90;
                        }
                        else if (cardsP1[p] >= 2)
                        {
                            returnScore[0] += p + 1 + 15;
                            timesP1++;
                            if (p > maxP1)
                            {
                                maxP1 = p;
            ...
            for (int l = 0; l < 5; l++)
            {
                for (int p = 0; p < 13; p++)
                {
                    if (player2Values[l] == p)
                    {
                        cardsP2[p]++;
                        if (cardsP2[p] >= 3)
                        {
                            returnScore[1] = p + 1 + 90;
                        }
                        else if(cardsP2[p] >= 2)
                        {
                            returnScore[1] += p + 1 + 15;
                            timesP2++;
                            if (p > maxP2)
                            {
                                maxP2 = p;
        ...
        public static int ScoreChecker(int gameNR)

        {
            ...
            player1Score = player1Score + CheckPairs(gameNR)[0];
            player2Score = player2Score + CheckPairs(gameNR)[1];

            if (player1Score == 0)
            {
                player1Score = player1Score + CheckHighestNumber(gameNR)[0];
            }
            if (player2Score == 0)
            {
                player2Score = player2Score + CheckHighestNumber(gameNR)[1];
            }


            if (player1Score > player2Score)
            {
                Console.WriteLine("Game: " + gameNR + " Player 1 Won Score: " + player1Score);
                Console.WriteLine("Score Player 2: " + player2Score);
                Console.WriteLine("____________________________________________________________");
                winner = 1;
                WriteToDisk.AddOutput(gameNR, WinState.PLAYER1);
            }
            else if (player2Score > player1Score)
            {
                Console.WriteLine("Game: " + gameNR + " Player 2 Won Score: " + player2Score);
                Console.WriteLine("Score Player 1: " + player1Score);
                Console.WriteLine("____________________________________________________________");
                winner = 2;
                WriteToDisk.AddOutput(gameNR, WinState.PLAYER2);
            }
            else
            {
                Console.WriteLine("Game: " + gameNR + " Draw, No player Won");
                Console.WriteLine("Score Player 1: " + player1Score);
                Console.WriteLine("Score Player 2: " + player2Score);
                Console.WriteLine("____________________________________________________________");
                winner = 3;
                WriteToDisk.AddOutput(gameNR, WinState.TIE);
            }
            return winner;
        }
